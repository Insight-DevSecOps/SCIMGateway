...
- [x] T090 [P] [US3] Define ITransformationEngine interface in src/SCIMGateway.Core/Models/Transformations/ITransformationEngine.cs ✅ (TransformGroupToEntitlementsAsync, TransformEntitlementToGroupsAsync, GetRulesAsync, CreateRuleAsync, UpdateRuleAsync, DeleteRuleAsync, TestRuleAsync, ResolveConflictsAsync, ValidateRuleAsync, PreviewTransformationAsync, RuleValidationResult)
- [x] T091 [US3] Implement TransformationEngine in src/SCIMGateway.Core/Models/Transformations/TransformationEngine.cs ✅ (load rules from cache/storage, match group displayName to rules by priority, apply transformation, handle conflicts)
- [x] T092 [US3] Implement EXACT pattern matching in TransformationEngine.cs ✅ (case-sensitive string match, direct mapping)
- [x] T093 [US3] Implement REGEX pattern matching in TransformationEngine.cs ✅ (regex pattern with capture groups, template variable substitution ${1}, ${2}, pre-compiled regex cache for performance)
- [x] T094 [US3] Implement HIERARCHICAL pattern matching in TransformationEngine.cs ✅ (path parsing with / separator, level extraction ${level0}, ${level1}, numbered group compat)
- [x] T095 [US3] Implement CONDITIONAL pattern matching in TransformationEngine.cs ✅ (CONTAINS, STARTS_WITH, ENDS_WITH, EQUALS, MATCHES operators, fallback support, regex fallback)
...
- [x] T120 [P] [US4] Implement SyncState storage in src/SyncEngine/SyncStateRepository.cs (create/update/query sync state in Cosmos DB sync-state container per contracts/cosmos-db-schema.md) ✅
